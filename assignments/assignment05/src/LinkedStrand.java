...
public class LinkedStrand implements DnaStrand {
    ...
    */
    public DnaStrand cutWith(String enzyme) {
        ...
        if(ind + enzyme.length() == front.dnaInfo.length()) {
    ...
    */
    public DnaStrand cutAndSplice(String enzyme, String splice) {
        ...
        if (ind < 0) {
        ...
        else if (enzyme.equals(this.front.dnaInfo)) {
        	return new LinkedStrand(splice);
        }
        	...
        	return new LinkedStrand(splice).append(new LinkedStrand(sub).cutAndSplice(enzyme, splice,this));
        }
        else {
        	String sub = this.start.dnaInfo.substring(0, ind);
        	String sub2 = this.start.dnaInfo.substring(ind + enzyme.length());
        	return new LinkedStrand(sub).append(new LinkedStrand(splice).append(new LinkedStrand(sub2).cutAndSplice(enzyme, splice,this)));
        }
    }
    
    public DnaStrand cutAndSplice(String enzyme, String splice, DnaStrand t) {
        int ind = this.start.dnaInfo.indexOf(enzyme);
        if (ind < 0) {
        	return this;
        }
    ...
    */
    ...
    public boolean thisThat(DnaStrand thiss, DnaStrand that) {
    	if(that != null) {
    		return false;
    	}
    	return true;
